...
plasticImg.src = 'https://i.ibb.co/Df9fmg8/plastic-small.png'; // Ensure the plastic image URL is correct
...
class Plastic {
    ...
    draw() {
        ctx.drawImage(plasticImg, this.frameX * this.spriteWidth, 0, this.spriteWidth, this.spriteHeight, this.x - 68, this.y - 68, this.spriteWidth * 1.5, this.spriteHeight * 1.5);
    }
}
...
function handlePlastic() {
    ...
    // Generate plastic periodically
    if (gameFrame % 200 == 0) { // You can adjust the frequency
...
function gameOver() {
    ...
    cancelAnimationFrame(animationFrame); // Stop the game
}
    ...
    for (let y = 0, y2 = textCoordinates.height; y < y2; y++) {
        for (let x = 0, x2 = textCoordinates.width; x < x2; x++) {
            if (textCoordinates.data[(y * 4 * textCoordinates.width) + (x * 4) + 3] > 128) {
                ...
                bubbleTextArray.push(new Particle2(positionX * 10, positionY * 10));
            }
        }
    }
}
...
function animate() {
    ...
    handleBubbles();
    handlePlastic();  // Handle plastic objects
